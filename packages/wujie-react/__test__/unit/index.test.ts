...
import WujieReact from "../../index";

function renderComponent(
  container: HTMLDivElement,
  props: Record<string, unknown>,
  forwardedRef: React.RefObject<WujieReactRef>
): void {
  act(() => {
    ReactDOM.render(React.createElement(WujieReact, { ...props, ref: forwardedRef } as never), container);
  });
}
...
describe("WujieReact", () => {
  ...
  test("mounts with every start option, restarts only for identity changes, and exposes controls", async () => {
    ...
    expect(controller.refresh).toHaveBeenLastCalledWith(expect.objectContaining({
      name: "second",
    ...
    const controls = forwardedRef.current as WujieReactRef;
    act(() => {
      ReactDOM.unmountComponentAtNode(host);
    });
  ...
  test("preserves the public static API", () => {
    expect(WujieReact.bus).toBe(mockBus);
    expect(WujieReact.setupApp).toBe(mockSetupApp);
    expect(WujieReact.preloadApp).toBe(mockPreloadApp);
    expect(WujieReact.destroyApp).toBe(mockDestroyApp);
    expect(WujieReact.refreshApp).toBe(mockRefreshApp);
    expect(WujieReact.clearAssetsCache).toBe(mockClearAssetsCache);
    expect(WujieReact.displayName).toBe("WujieReact");
    expect("propTypes" in WujieReact).toBe(false);
  });
  ...
  test("reports synchronous automatic-start failures and disposes the controller", async () => {
    ...
    renderComponent(host, { name: "broken" }, React.createRef<WujieReactRef>());
    await flushPromises();

    expect(consoleError).toHaveBeenCalledWith("[wujie-react] failed to start application", failure);
    act(() => {
      ReactDOM.unmountComponentAtNode(host);
    });
    expect(controller.dispose).toHaveBeenCalledTimes(1);
  ...
  test("reports controller-construction failures and safely unmounts without a controller", async () => {
    ...
    renderComponent(host, { name: "broken-controller" }, React.createRef<WujieReactRef>());
    await flushPromises();
