...
describe('sandbox lifecycle races', () => {
  ...
  test('async child unmount can reenter public destroy after an await', async () => {
    ...
    const previousPowered = window.__POWERED_BY_WUJIE__;
    const previousOwner = window.__WUJIE;
    window.__POWERED_BY_WUJIE__ = true;
    window.__WUJIE = sandbox;
    try {
      await expect(destroyApp(name)).resolves.toBeUndefined();
    } finally {
      window.__POWERED_BY_WUJIE__ = previousPowered;
      window.__WUJIE = previousOwner;
    }
  ...
  test('a concurrent public destroy acknowledges ownership while the first teardown continues', async () => {
    ...
    sandbox.mountFlag = true;
    sandbox.iframe.contentWindow.__WUJIE_UNMOUNT = () => gate.promise;
