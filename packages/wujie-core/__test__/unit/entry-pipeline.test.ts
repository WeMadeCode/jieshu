...
import type { WujiePlugin } from "../../src/contracts";
...
describe("entry asset pipeline", () => {
  ...
  test.each(stylesheetFailureCases)(
    ...
    async (failureKind, fail) => {
      ...
      const retryFetch = jest.fn(
        (_input: RequestInfo): Promise<Response> => Promise.resolve(response("body { color: green; }"))
      );
  ...
  test("forwards applicable link and inline-style attributes into successful embedded styles", async () => {
    ...
    const fetch = jest.fn(
      (_input: RequestInfo): Promise<Response> => Promise.resolve(response(".external { color: green; }"))
    );
    const parsed = await importHTML({
      url: entryUrl,
      html: [
  ...
  test("keeps a disabled stylesheet as a native link with a resolved child URL", async () => {
    ...
    const fetch = jest.fn(
      (_input: RequestInfo): Promise<Response> => Promise.resolve(response(".disabled { color: red; }"))
    );
    const parsed = await importHTML({
      url: entryUrl,
  ...
  test("restores a plugin-ignored stylesheet with its resolved href and original attributes", async () => {
    ...
    const fetch = jest.fn(
      (_input: RequestInfo): Promise<Response> => Promise.resolve(response(".ignored { color: red; }"))
    );
    const parsed = await importHTML({
      url: entryUrl,
