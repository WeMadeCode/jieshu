...
export function assertResolvedStartOptions(options: ResolvedOptions): asserts options is ResolvedStartOptions {
  if (!options.url) throw new TypeError(`Wujie application "${options.name}" requires a url`);
  if (options.el === undefined) throw new TypeError(`Wujie application "${options.name}" requires a container`);
}
