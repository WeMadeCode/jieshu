...
export type WujieVueComponent = DefineComponent<WujieVueProps> &
  WujieVueStatics & {
...
const component = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const currentOptions = (): StartOptions => optionsFromProps(props, container.value);
    ...
    // Vue 3.0 exposes setup returns on the public instance. The named ref also
    // works before setupContext.expose was introduced in Vue 3.1.
    return { wujieContainer: container, refresh, destroy };
  },
...
// Vue marks runtime props with defaults as optional for template consumers,
// while the adapter's public contract keeps the application name required.
// The runtime component and the separately checked static API are therefore
// joined through an explicit public facade.
const WujieVue = component as unknown as WujieVueComponent;
WujieVue.bus = bus;
