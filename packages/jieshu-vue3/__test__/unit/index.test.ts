...
type ExposedInstance = ComponentPublicInstance & JieshuVueExposed;
...
function mountComponent(
  ...
  const Root = defineComponent({
    setup() {
      return () => h(JieshuVue as never, { ...state, ...listeners, ref: child } as never);
    },
  });
...
describe('JieshuVue for Vue 3', () => {
  ...
  test('uses default styles and preserves the plugin/static API', () => {
    ...
    expect(JieshuVue.bus).toBe(mockBus);
    expect(JieshuVue.setupApp).toBe(mockSetupApp);
    expect(JieshuVue.preloadApp).toBe(mockPreloadApp);
    expect(JieshuVue.destroyApp).toBe(mockDestroyApp);
    expect(JieshuVue.refreshApp).toBe(mockRefreshApp);
    expect(JieshuVue.clearAssetsCache).toBe(mockClearAssetsCache);

    const app = { component: vi.fn() } as unknown as App;
    JieshuVue.install(app);
    expect(app.component).toHaveBeenCalledWith('JieshuVue', expect.anything());
  });
  ...
  test('guards pre-mount identity changes and supports import without HTMLElement', async () => {
    ...
    try {
      ...
      const isolatedComponent = (await import('../../src')).default as unknown as {
        setup(props: Record<string, unknown>, context: { emit: Mock }): JieshuVueExposed;
      };
