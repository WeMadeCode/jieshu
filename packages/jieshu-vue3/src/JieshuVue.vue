<template>
  <div ref="jieshuContainer" :style="{ width, height, ...style }"></div>
</template>
...
type ResolvedComponentProps = Readonly<{
  [Key in keyof typeof componentProps]: Key extends 'name' | 'url' | 'width' | 'height'
    ? string
    : Key extends keyof import('./index').JieshuVueProps
      ? import('./index').JieshuVueProps[Key]
      : never;
}>;

function optionsFromProps(props: ResolvedComponentProps, container: HTMLElement | null): StartOptions {
  if (!container) throw new Error('JieshuVue cannot start before its container is mounted');
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const jieshuContainer = ref<HTMLElement | null>(null);
    const controller = createAppController();
    let mounted = false;

    const currentOptions = (): StartOptions => optionsFromProps(props, jieshuContainer.value);
    ...
    return { jieshuContainer, refresh, destroy };
  },
});
</script>
