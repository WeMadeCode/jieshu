...
export default class Jieshu {
  ...
  /** 子应用的插件 */
  public plugins: Array<JieshuPlugin>;
  /** js沙箱ready态 */
  ...
  public async active(options: {
    ...
    this.provide.props = (props as InjectedJieshuProps | undefined) ?? this.provide.props;
  ...
  public async start(getExternalScripts: () => ScriptResultList): Promise<void> {
    ...
    const iframeWindow = getIframeWindow(this.iframe);
    // 标志位，执行代码前设置
    iframeWindow.__POWERED_BY_JIESHU__ = true;
  ...
  /**
   * 框架主动发起mount，如果子应用是异步渲染实例，比如将生命周__JIESHU_MOUNT放到async函数内
   * 此时如果采用fiber模式渲染（主应用调用mount的时机也是异步不确定的），框架调用mount时可能
   * 子应用的__JIESHU_MOUNT还没有挂载到window，所以这里封装一个mount函数，当子应用是异步渲染
   * 实例时，子应用异步函数里面最后加上window.__JIESHU.mount()来主动调用
   */
  public mount(advanceQueue = true): void {
    ...
    // A delayed child-side `window.__JIESHU.mount()` may arrive after the host
  ...
  public unmount(): Promise<void> {
    ...
    // Removing an already-released <jieshu-app> can synchronously invoke its
  ...
  private async performUnmount(): Promise<void> {
    ...
    if (!this.mountFlag) return;
    const iframeWindow = getIframeWindow(this.iframe);
    if (isFunction(iframeWindow.__JIESHU_UNMOUNT) && !this.alive && !this.hrefFlag) {
      this.lifecycles?.beforeUnmount?.(iframeWindow);
      await invokeSandboxUnmountHook(this.id, () => iframeWindow.__JIESHU_UNMOUNT());
  ...
  public destroy(): Promise<void> {
    ...
    this.clearContainerOnDestroy = getJieshuById(this.id) === this;
    // 同步从全局 map 移除自身：确保 await 挂起期间并发的 disconnectedCallback /
    // startApp 通过 getJieshuById 拿到 null，避免对同一实例重复 destroy（竞态根因）。
    deleteJieshuById(this.id, this);
  ...
  private releaseIframe(): void {
    ...
    if (iframeWindow) {
      try {
        ...
        Reflect.set(iframeWindow, '$jieshu', null);
      } catch (_) {
   ...
   */
  private releaseReference(property: keyof Jieshu): void {
   ...
   * 使用 JIESHU_APP_ID 标识属于当前子应用的 font 样式。
   */
  ...
  private createFontStyleSheetContainer(): HTMLElement {
    const container = rawDocumentQuerySelector.call(document, `[${JIESHU_FONT_STYLE_CONTAINER_ATTR}]`);
    if (container) return container as HTMLElement;

    const styleElement = document.createElement('style');
    styleElement.setAttribute(JIESHU_FONT_STYLE_CONTAINER_ATTR, '');
  ...
  public patchCssRules(): void {
    ...
    if (fontStyleSheetElement) {
      ...
      fontStyleSheetElement.setAttribute(JIESHU_APP_ID, this.id);
      this.fontStyleSheetElements.push(fontStyleSheetElement);
    }
    (hostStyleSheetElement || fontStyleSheetElement) &&
      this.shadowRoot.host.setAttribute(JIESHU_DATA_ATTACH_CSS_FLAG, '');
  }
