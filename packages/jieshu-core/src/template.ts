...
function decodeAttributeEntities(value: string): string {
  ...
  decoder.innerHTML = `<span data-jieshu-value="${quotedValue}"></span>`;
  return decoder.firstElementChild?.getAttribute('data-jieshu-value') ?? value;
}
...
export const genLinkReplaceSymbol = (linkHref: string, preloadOrPrefetch = false): string =>
  `<!-- ${preloadOrPrefetch ? 'prefetch/preload/modulepreload' : ''} link ${linkHref} replaced by jieshu -->`;
export const getInlineStyleReplaceSymbol = (index: number): string =>
  `<!-- inline-style-${index} replaced by jieshu -->`;
export const genScriptReplaceSymbol = (scriptSrc: string, type = ''): string =>
  `<!-- ${type} script ${scriptSrc} replaced by jieshu -->`;
export const inlineScriptReplaceSymbol = '<!-- inline scripts replaced by jieshu -->';
export const genIgnoreAssetReplaceSymbol = (url: string): string =>
  `<!-- ignore asset ${url || 'file'} replaced by jieshu -->`;
export const genModuleScriptReplaceSymbol = (scriptSrc: string, moduleSupport: boolean): string =>
  `<!-- ${moduleSupport ? 'nomodule' : 'module'} script ${scriptSrc} ignored by jieshu -->`;
