...
import { JIESHU_ALL_EVENT, JIESHU_TIPS_NO_SUBJECT } from './constant';
...
function createEventRegistry(): EventRegistry {
  ...
  const inheritedRegistry = runtimeWindow.__JIESHU_INJECT?.appEventObjMap;
  if (inheritedRegistry) return inheritedRegistry;

  const registry = runtimeWindow.__POWERED_BY_JIESHU__
    ? runtimeWindow.__JIESHU!.inject.appEventObjMap
    : new Map<string, EventObj>();

  runtimeWindow.__JIESHU_INJECT = {
    ...runtimeWindow.__JIESHU_INJECT,
...
export class EventBus {
  ...
  public $emit<Arguments extends unknown[]>(event: string, ...args: Arguments): EventBus {
    ...
    appEventObjMap.forEach((eventObj) => {
      ...
      allEventListeners.push(...listenersFor(eventObj, JIESHU_ALL_EVENT));
    });
