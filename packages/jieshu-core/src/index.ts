...
function isChildSelfUnmountOperation(id: string): boolean {
  try {
    const owner = window.__JIESHU;
    return Boolean(
      window.__POWERED_BY_JIESHU__ &&
      owner?.id === id &&
...
function isSandboxUnavailable(sandbox: Jieshu, canContinue: ContinuationGuard): boolean {
  return (
    ...
    getJieshuById(sandbox.id) !== sandbox
  );
}
...
async function retainOnlyActiveInitialization(sandbox: Jieshu, canContinue: ContinuationGuard): Promise<boolean> {
...
/**
 * 强制中断主应用运行
 * jieshu.__JIESHU 如果为true说明当前运行环境是子应用
 * window.__POWERED_BY_JIESHU__ 如果为false说明子应用还没初始化完成
 * 上述条件同时成立说明主应用代码在iframe的loading阶段混入进来了，必须中断执行
 */
if (window.__JIESHU && !window.__POWERED_BY_JIESHU__) {
  stopMainAppRun();
}
...
export function preloadApp(preOptions: PreOptions): void {
  ...
  requestIdleCallback(() => {
    const preload = async (): Promise<void> => {
       ...
       */
      if (getJieshuById(request.name) || isMatchSyncQueryById(request.name)) return;
...
async function destroyAppNow(id: string, canContinue: ContinuationGuard): Promise<void> {
  const sandbox = getJieshuById(id);
  if (sandbox) {
    await sandbox.destroy();
    return;
  }
  ...
  const replacement = getJieshuById(id);
