const JIESHU_INLINE_EVENT_PREFIX = '{const __JIESHU_INLINE_SCOPE__=window.__getJieshuWindow__(';
const LEGACY_INLINE_EVENT_PREFIX = 'with(window.__getJieshuWindow__(';
...
export function wrapInlineEventHandler(handler: string, appId: string): string {
  if (handler.startsWith(JIESHU_INLINE_EVENT_PREFIX) || handler.startsWith(LEGACY_INLINE_EVENT_PREFIX)) return handler;
  return `${JIESHU_INLINE_EVENT_PREFIX}${JSON.stringify(
    appId,
  )});if(__JIESHU_INLINE_SCOPE__){with(__JIESHU_INLINE_SCOPE__){ ${handler} }}}`;
}
