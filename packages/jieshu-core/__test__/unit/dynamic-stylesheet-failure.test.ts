...
function createSandbox(
  ...
  Reflect.set(iframe.contentWindow as Window, '__JIESHU', sandbox);
...
describe('dynamic stylesheet failures', () => {
  ...
  test('dispatches error instead of load and leaves a failed request retryable', async () => {
    ...
    expect(root.head.querySelector('style[data-jieshu-css-href]')?.textContent).toContain('color: green');
  });
