...
import * as jieshu from '../../src/index';
...
describe('clearAssetsCache 资源缓存清理 API', () => {
  ...
  test('公共入口 jieshu.clearAssetsCache 应被导出', () => {
    expect(typeof jieshu.clearAssetsCache).toBe('function');
  });
  ...
  test('clearAssetsCache(host) 应只清空匹配 host 前缀的 entry', () => {
    ...
    jieshu.clearAssetsCache('http://a.com');
  ...
  test('clearAssetsCache([h1, h2]) 应支持批量 host', () => {
    ...
    jieshu.clearAssetsCache(['http://a.com', 'http://c.com']);
