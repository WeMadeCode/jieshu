...
import { getJieshuWindow } from '../../src/entry';
import { JIESHU_DATA_FLAG } from '../../src/constant';

function createIframeWindow(appId = 'inline-app'): { iframe: HTMLIFrameElement; iframeWindow: Window } {
  const iframe = document.createElement('iframe');
  iframe.setAttribute(JIESHU_DATA_FLAG, '');
  document.body.appendChild(iframe);
  const iframeWindow = iframe.contentWindow as Window;
  iframeWindow.__JIESHU = { id: appId } as unknown as Window['__JIESHU'];
...
describe('iframe inline event compiler', () => {
  ...
  it('recursively compiles handlers once while preserving ordinary attributes', () => {
    ...
    expect(compiled).toBe(
      '{const __JIESHU_INLINE_SCOPE__=window.__getJieshuWindow__("inline-app");if(__JIESHU_INLINE_SCOPE__){with(__JIESHU_INLINE_SCOPE__){ run() }}}',
    );
    expect(span.getAttribute('onmouseover')).toBe(
      '{const __JIESHU_INLINE_SCOPE__=window.__getJieshuWindow__("inline-app");if(__JIESHU_INLINE_SCOPE__){with(__JIESHU_INLINE_SCOPE__){ hover() }}}',
    );
  ...
  it('compiles dynamically assigned event attributes through the patched setter', () => {
    ...
    expect(element.getAttribute('onclick')).toBe(
      '{const __JIESHU_INLINE_SCOPE__=window.__getJieshuWindow__("dynamic-app");if(__JIESHU_INLINE_SCOPE__){with(__JIESHU_INLINE_SCOPE__){ submitForm() }}}',
    );
  ...
  it('normalizes mixed-case and attribute-node event writes', () => {
    ...
    expect(element.getAttribute('onclick')).toContain('window.__getJieshuWindow__("attribute-api-app")');
    expect(element.getAttribute('onmouseover')).toContain('window.__getJieshuWindow__("attribute-api-app")');
    expect(element.getAttribute('onfocus')).toContain('window.__getJieshuWindow__("attribute-api-app")');
  });
  ...
  it('compiles handlers introduced by dynamic markup APIs', () => {
    ...
    expect(root.querySelector('#inner')?.getAttribute('onclick')).toContain('window.__getJieshuWindow__("markup-app")');
    expect(root.querySelector('#adjacent')?.getAttribute('onclick')).toContain(
      'window.__getJieshuWindow__("markup-app")',
    );
  ...
  it('compiles handlers on nodes created by iframe-realm outerHTML', () => {
    ...
    expect(root.querySelector('#replacement')?.getAttribute('onclick')).toContain(
      'window.__getJieshuWindow__("outer-html-app")',
    );
  });
  ...
  it('executes a dynamic click in the child scope instead of the raw iframe global', () => {
    ...
    iframeWindow.__getJieshuWindow__ = () => ({ runDynamicHandler: childHandler }) as unknown as WindowProxy;
    patchInlineEventSetAttribute(iframeWindow);
    const root = iframeWindow.document.createElement('section');
  ...
  it('leaves an already compiled handler unchanged', () => {
    const compiled =
      '{const __JIESHU_INLINE_SCOPE__=window.__getJieshuWindow__("app");if(__JIESHU_INLINE_SCOPE__){with(__JIESHU_INLINE_SCOPE__){ work() }}}';
