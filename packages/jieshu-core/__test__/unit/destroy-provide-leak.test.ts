/**
 * 单元测试：sandbox.destroy() 对 iframeWindow.$jieshu 的清理契约。
 *
 * provide 与 iframeWindow.$jieshu 指向同一对象（patchIframeVariable）。destroy 时须在
 * iframeWindow 上把 __JIESHU、$jieshu 一并置 null；仅 this.provide = null 只断了 sandbox
...
describe('sandbox.destroy() 对 iframeWindow.$jieshu 的清理契约', () => {
  function createDestroyableSandbox() {
    ...
    iframeWindow.__JIESHU = { id: 'leak-test' };
    iframeWindow.$jieshu = provide;
  ...
  test('destroy 后 iframeWindow.$jieshu 与 __JIESHU 均被断链为 null', async () => {
    const { inst, iframeWindow } = createDestroyableSandbox();

    await inst.destroy();

    expect(iframeWindow.$jieshu).toBeNull();
    expect(iframeWindow.__JIESHU).toBeNull();
  });
