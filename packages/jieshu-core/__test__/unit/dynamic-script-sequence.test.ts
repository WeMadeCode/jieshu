...
function createSandbox(id: string, fetch: (input: RequestInfo) => Promise<Response>): Jieshu {
  ...
  Reflect.set(iframeWindow, '__JIESHU', sandbox);
...
describe('dynamic script sequencing', () => {
  ...
  test('an inline module uses native module completion and forwards its error', async () => {
    ...
    expect(injectedModule.textContent).not.toContain('.bind(window.__JIESHU.proxy)');
    expect(loaded).not.toHaveBeenCalled();
  ...
  test('a cssLoader that synchronously unmounts cannot resurrect its placeholder', async () => {
    ...
    expect(root.head.querySelector('style[data-jieshu-css-href]')).toBeNull();
  });
