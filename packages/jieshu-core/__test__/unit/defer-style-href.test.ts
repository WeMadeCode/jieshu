 ...
 * jieshu 旧实现会直接用注释替换并丢弃该 link，导致样式（skin.min.css）永远加载不出来。
 *
...
import { addSandboxCacheWithJieshu, deleteJieshuById } from '../../src/common';
import { deferStyleSheetByHref } from '../../src/effect';

const JIESHU_ID = 'defer-style-href-app';

function createSandbox(): Jieshu {
  return {
    id: JIESHU_ID,
...
describe("deferStyleSheetByHref / 先 append 后 setAttribute('href') 的延迟加载", () => {
  ...
  beforeEach(() => {
    ...
    addSandboxCacheWithJieshu(JIESHU_ID, sandbox);
  });

  afterEach(() => {
    deleteJieshuById(JIESHU_ID);
    vi.useRealTimers();
  });
  ...
  it("后续 setAttribute('href') 应触发 loadStyleSheet 并带上真实 href", async () => {
    ...
    deferStyleSheetByHref({ element: link, jieshuId: JIESHU_ID, iframeWindow: window, loadStyleSheet });
  ...
  it('环境不支持 MutationObserver 时应安全跳过，不抛错也不入队', () => {
    ...
    expect(() =>
      deferStyleSheetByHref({ element: link, jieshuId: JIESHU_ID, iframeWindow: fakeWindow, loadStyleSheet }),
    ).not.toThrow();
    expect(sandbox.deferredStyleObservers).toHaveLength(0);
  });
});
