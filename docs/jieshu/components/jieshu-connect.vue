...
function changeJieshuUrl(item: { url: string }, index: number) {
  flag.value = !flag.value;
...
<template>
  <section id="newsletter" class="NewsLetter">
    <div class="container">
      <h2 class="title"><span class="link">开箱即用</span> , 用最简单的方式体验<span class="link">界枢</span></h2>
      <div class="form">
      ...
      <p class="help">
        您可以输入一个允许跨域访问的 <span class="link">https</span> 协议网站来在线体验 <span class="link">界枢</span>
      </p>
    </div>
    <div class="jieshuList">
      <span>快速前往:</span>
      <div
        v-for="(item, index) in jieshuList"
        :key="item.url"
        :class="['jieshuItem', { chooseItem: index === currentIndex }]"
        @click="changeJieshuUrl(item, index)"
      >
        {{ item.name }}
      </div>
    </div>
